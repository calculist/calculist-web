...
import urlFinder from '../../client/services/urlFinder';
import itemsToSVG from '../utility/itemsToSVG';

const createComputationContextObject = (function (_, ss, d3, evalculist, isItem, keyToVarName, getItemByGuid, urlFinder, itemsToSVG) {
  ...
  proto.bar = function (num, denom, text) {
    text = _.escape(" " + (text || ""));
    return {
      toString: function () { return "[Bar]"; },
      toHTML: function () {
        return '<span class="bar-background">' +
          '<span class="bar" style="width:' + (98.7 * (num / denom)) + '%">' +
            '<span style="opacity:0;">.</span>' + // invisible char to force non-zero height
            text +
          '</span>' +
        '</span>';
      }
  ...
  proto.dots = function (blueDotCount, totalDotCount) {
    totalDotCount || (totalDotCount = blueDotCount);
    return {
      toString: function () { return '[Dots]'; },
      toHTML: function () {
        return _.times(totalDotCount, function (i) {
          var color = i < blueDotCount ? 'blue' : 'gray';
          return '<span class="big-dot big-' + color + '-dot"></span>';
        }).join('');
      }
    };
  };
...
})(_, ss, d3, evalculist, isItem, keyToVarName, getItemByGuid, urlFinder, itemsToSVG);

export default createComputationContextObject;
