/**
 * Web Worker for computing diffs between item arrays.
 */

var getDiff = function (a: any, b: any, keys: string[]) {
  var diff: any;
  if (a && !b) {
    diff = [a, 0, 0];
  } else if (!a && b) {
...
(self as any).onmessage = function(e: MessageEvent) {
  ...
  var keys = Object.keys(e.data[0][0]).reduce(function (keys: string[], key: string) {
  ...
  (self as any).postMessage(changes);
};
